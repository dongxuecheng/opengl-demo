...
def test_incremental_render():
    ...
    init_response = requests.post(
        f"{API_BASE}/render/init", json={"template": TEMPLATE, "image_path": IMAGE_PATH}
    )
    ...
    append1_response = requests.post(
        ...
        json={"session_id": session_id, "video_path": VIDEO_PATHS[0]},
    )
    ...
    append2_response = requests.post(
        f"{API_BASE}/render/append",
        json={"session_id": session_id, "video_path": VIDEO_PATHS[1]},
    )
    ...
    finalize_response = requests.post(
        f"{API_BASE}/render/finalize", json={"session_id": session_id}
    )
